...
async function main() {
    ...
    const delegateVoteToAdress = process.argv[3];
    
    const provider = ethers.getDefaultProvider("goerli", {alchemy: process.env.ALCHEMY_API_KEY});
//    const wallet = ethers.Wallet.fromMnemonic(process.env.MNEMONIC ?? "");
    const wallet = new ethers.Wallet(process.env.PRIVATE_KEY_ACC2 ?? "");
    ...
    const ballotContract = ballotContractFactory.attach(
        contractAdress);
    const tx = await ballotContract.delegate(delegateVoteToAdress);
    await tx.wait();
